...
def plot_resultado_files(file_path):
    ...
    try:
        ...
        plt.savefig(os.path.join(os.path.dirname(file_path), f'{base_name}_duracao_vs_inicio.png'))
        plt.close()
...
def process_file(file_path):
    ...
    elif file_name.startswith('resultado') and file_name.endswith('.csv'):
...
def main():
    ...
    files_to_process = []
    for target_dir in ['dia1v2', 'dia1v3', 'dia2v2']:
        if not os.path.isdir(target_dir):
            print(f"Diretório '{target_dir}' não encontrado. Pulando.")
            continue
        for root, _, files in os.walk(target_dir):
            for file in files:
                if 'dados_geral' in file or (file.startswith('resultado') and file.endswith('.csv')) or file == 'db_monitoramento_nivel.csv':
    ...
    # Usa todos os núcleos disponíveis, mas deixa 1 livre para o SO não travar
